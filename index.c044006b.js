...
Y="(?:\\\\[\\da-fA-F]{1,6}"+D+"?|\\\\[^\\r\\n\\f]|[\\w-]|[^\x00-\\x7f])+",O="\\["+D+"*("+Y+")(?:"+D+// Operator (capture 2)
"*([*^$|!~]?=)"+D+// "Attribute values must be CSS identifiers [capture 5] or strings [capture 3 or capture 4]"
"*(?:'((?:\\\\.|[^\\\\'])*)'|\"((?:\\\\.|[^\\\\\"])*)\"|("+Y+"))|)"+D+"*\\]",z=":("+Y+")(?:\\((('((?:\\\\.|[^\\\\'])*)'|\"((?:\\\\.|[^\\\\\"])*)\")|((?:\\\\.|[^\\\\()[\\]]|"+O+")*)|.*)\\)|)",M=RegExp(D+"+","g"),N=RegExp("^"+D+"*,"+D+"*"),R=RegExp("^"+D+"*([>+~]|"+D+")"+D+"*"),j=RegExp(D+"|>"),W=new RegExp(z),B=RegExp("^"+Y+"$"),q={ID:RegExp("^#("+Y+")"),CLASS:RegExp("^\\.("+Y+")"),TAG:RegExp("^("+Y+"|[*])"),ATTR:RegExp("^"+O),PSEUDO:RegExp("^"+z),CHILD:RegExp("^:(only|first|last|nth|nth-last)-(child|of-type)(?:\\("+D+"*(even|odd|(([+-]|)(\\d*)n|)"+D+"*(?:([+-]|)"+D+"*(\\d+)|))"+D+"*\\)|)","i"),bool:RegExp("^(?:"+L+")$","i"),// For use in libraries implementing .is()
...
"string"==typeof t&&Y.test(t)?T(t):t||[],!1).length}});// Initialize a jQuery object
...
if(!Y.test(t)){for(;n<o;n++)for(i=this[n];i&&i!==e;i=i.parentNode)if(i.nodeType<11&&(r?r.index(i)>-1:1===i.nodeType&&T.find.matchesSelector(i,t))){s.push(i);break}}return this.pushStack(s.length>1?T.uniqueSort(s):s)},// Determine the position of an element within the set
...
function tY(t){return t.type=(null!==t.getAttribute("type"))+"/"+t.type,t}function tO(t){return"true/"===(t.type||"").slice(0,5)?t.type=t.type.slice(5):t.removeAttribute("type"),t}function tz(t,e){var i,n,o,s,r,a;if(1===e.nodeType){// 1. Copy private data: events, handlers, etc.
...
for(l=(a=T.map(tw(o,"script"),tY)).length;u<p;u++)c=o,u!==d&&(c=T.clone(c,!0,!0),l&&// push.apply(_, arraylike) throws on ancient WebKit
...
// Fixes trac-9887
function eY(t,e){var i,n,o=T.ajaxSettings.flatOptions||{};for(i in e)void 0!==e[i]&&((o[i]?t:n||(n={}))[i]=e[i]);return n&&T.extend(!0,t,n),t}eH.href=ef.href,T.extend({// Counter for holding the number of active queries
...
ajaxSetup:function(t,e){return e?eY(eY(t,T.ajaxSettings),e):eY(T.ajaxSettings,t)},ajaxPrefilter:eX(eD),ajaxTransport:eX(eI),// Main method
...
td=!0,tp=U.height(),t(v).each(function(){var e=t(this).find(_),i=t(this).find(A);M.verticalCentered&&t(this).find(w).css("height",ex(t(this))+"px"),t(this).css("height",tp+"px"),i.length>1&&ec(e,e.find(D))}),M.scrollOverflow&&Z.createScrollBarForAll();var i=t(y).index(v);i&&tI(i+1),td=!1,t.isFunction(M.afterResize)&&e&&M.afterResize.call(tu),t.isFunction(M.afterReBuild)&&!e&&M.afterReBuild.call(tu)}}/**
        ...
        */function tY(e){var i=tr.hasClass(a);e?!i&&(tS(!1,"internal"),tE(!1,"internal"),t(E).hide(),tr.addClass(a),t.isFunction(M.afterResponsive)&&M.afterResponsive.call(tu,e)):i&&(tS(tb.autoScrolling,"internal"),tE(tb.autoScrolling,"internal"),t(E).show(),tr.removeClass(a),t.isFunction(M.afterResponsive)&&M.afterResponsive.call(tu,e))}/**
...
ta.version="2.9.6",ta.setAutoScrolling=tS,ta.setRecordHistory=tw,ta.setScrollingSpeed=tT,ta.setFitToSection=tE,ta.setLockAnchors=/**
        * Sets lockAnchors
        */function(t){M.lockAnchors=t},ta.setMouseWheelScrolling=tk,ta.setAllowScrolling=tP,ta.setKeyboardScrolling=tC,ta.moveSectionUp=tA,ta.moveSectionDown=tD,ta.silentMoveTo=tI,ta.moveTo=t_,ta.moveSlideRight=tH,ta.moveSlideLeft=tX,ta.fitToSection=tR,ta.reBuild=tL,ta.setResponsive=tY,ta.destroy=/*
...
s.data("fp-styles",s.attr("style")),e||0!==t(y).length||s.addClass(p),Q=t(y),s.css("height",tp+"px"),M.paddingTop&&s.css("padding-top",M.paddingTop),M.paddingBottom&&s.css("padding-bottom",M.paddingBottom),void 0!==M.sectionsColor[e]&&s.css("background-color",M.sectionsColor[e]),void 0!==M.anchors[e]&&s.attr("data-anchor",M.anchors[e]),void 0!==M.anchors[e]&&s.hasClass(p)&&eg(M.anchors[e],e),M.menu&&M.css3&&t(M.menu).closest(r).length&&t(M.menu).appendTo(tr),l>0?(i=100*l,n=100/l,a.wrapAll('<div class="'+H+'" />'),a.parent().wrap('<div class="'+I+'" />'),s.find(X).css("width",i+"%"),l>1&&(M.controlArrows&&(s.find(_).after('<div class="'+W+'"></div><div class="'+F+'"></div>'),"#fff"!=M.controlArrowColor&&(s.find(V).css("border-color","transparent transparent transparent "+M.controlArrowColor),s.find(B).css("border-color","transparent "+M.controlArrowColor+" transparent transparent")),M.loopHorizontal||s.find(B).hide()),M.slidesNavigation&&/**
        ...
        */function(t,e){t.append('<div class="'+Y+'"><ul></ul></div>');var i=t.find(O);//top or bottom
...
"all"!==i?tv[n][i]=e:t.each(Object.keys(tv[n]),function(t,i){tv[n][i]=e})}/*
        ...
        * Displays warnings
        */function eL(){if(t("html").hasClass(h)){eY("error","Fullpage.js can only be initialized once and you are doing it multiple times!");return}M.continuousVertical&&(M.loopTop||M.loopBottom)&&(M.continuousVertical=!1,eY("warn","Option `loopTop/loopBottom` is mutually exclusive with `continuousVertical`; `continuousVertical` disabled")),M.scrollBar&&M.scrollOverflow&&eY("warn","Option `scrollBar` is mutually exclusive with `scrollOverflow`. Sections with scrollOverflow might not work well in Firefox"),M.continuousVertical&&(M.scrollBar||!M.autoScrolling)&&(M.continuousVertical=!1,eY("warn","Scroll bars (`scrollBar:true` or `autoScrolling:false`) are mutually exclusive with `continuousVertical`; `continuousVertical` disabled")),M.scrollOverflow&&!M.scrollOverflowHandler&&(M.scrollOverflow=!1,eY("error","The option `scrollOverflow:true` requires the file `scrolloverflow.min.js`. Please include it before fullPage.js.")),//using extensions? Wrong file!
